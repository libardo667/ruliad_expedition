  ...
  const ruleSentence=(()=>{
    if(derivationType==="similarity_matrix"){
      if(topDistancePair){
        ...
        return `This run's tension topology maps to Rule ${rule} (${classLabel}), derived from disc-similarity extremes${median}; the highest-tension pair is ${a} vs ${b} (distance ${dist}).`;
      }
      return `This run's tension topology maps to Rule ${rule} (${classLabel}), derived from disc-similarity distance extremes.`;
    }
    if(derivationType==="fallback_hash"){
      return `This run maps to Rule ${rule} (${classLabel}) via deterministic fallback hashing because similarity-matrix diagnostics were unavailable.`;
    }
    return `This run uses Rule ${rule} (${classLabel}) under ${derivationType.replace(/_/g," ")} derivation.`;
  })();
